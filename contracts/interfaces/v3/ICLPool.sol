...
interface ICLPool {
     ...
     * initialized Set to true if the tick is initialized (i.e. liquidityGross is greater than 0), otherwise false.
     * Outside values can only be used if the tick is initialized. In addition, these values are only relative and must
     * be used only in comparison to previous snapshots for a specific position.
     */
    function ticks(
        int24 tick
    )
     ...
     * @dev Can be called once only. Price is represented as a sqrt(amountToken1/amountToken0) Q64.96 value.
