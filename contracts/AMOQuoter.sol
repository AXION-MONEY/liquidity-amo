...
contract AMOQuoter {
    ...
    function revertCall(address amo, uint256 liquidity) external {
    ...
    function afterUBB(
        ...
        try this.revertCall(amo, liquidity) {} catch (bytes memory reason) {
            assembly {
                reason := add(reason, 4)
            }
    ...
    function bestLiquidity(
        ...
        for (uint i; i < iters; i++) {
            (liquidityUsed, excessiveLiquidity, newPrice) = afterUBB(amo, liquidity);
