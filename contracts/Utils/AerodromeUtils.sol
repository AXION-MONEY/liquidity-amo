...
contract AerodromeUtils is MasterUtils {
    ...
    function checkIncreaseUnlockTime(uint256 tokenId, uint256 lockDuration) public override view returns (bool) {
    ...
    function increaseAmount(
        uint256 tokenId,
        uint256 value
    ) external override nonReentrant onlyRole(VOTER_ROLE) {
        IveAERO(ve).increaseAmount(tokenId, value);
    }

    function getVoterVe(address voter_) public override view returns (address) {
