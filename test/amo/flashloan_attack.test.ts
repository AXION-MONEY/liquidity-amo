...
describe("Flashloan attack scenarios", function () {
  describe("USD Flashloan", function () {
    it("attacker cannot profit or permanently distort price via a USD flashloan", async () => {
      ...
      await expect(usd.connect(attacker).transfer(minter.target, flashAmt)).to.be.revertedWithCustomError(
        usd,
  ...
  describe("ION Flashloan", function () {
    it("attacker cannot profit or permanently distort price via an ION flash‑loan", async () => {
      ...
      // Repay flashloan will revert due to insufficient balance
      await expect(ion.connect(attacker).transfer(admin.address, flashAmtION)).to.be.revertedWithCustomError(
        ion,
        "ERC20InsufficientBalance"
      );
    });
...
describe("Whale attack arbitrage on a price dip", function () {
  it("attacker profits by buying low and selling after AMO buyback", async () => {
    ...
    expect(initialPortfolioUsd).to.be.gt(usdEnd, "attacker made profit after the roundtrip");
  });
});
