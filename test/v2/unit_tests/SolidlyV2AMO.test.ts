...
describe("V2AMO", function () {
  ...
  async function setupSolidlyV2Environment() {
    try {
      ...
      const createPairTx = await factory.connect(admin).createPair(boostAddress, usdAddress, stable);
      ...
      await network.provider.send("hardhat_setBalance", [governor, "0x1000000000000000000"]);
      ...
      try {
        await v2Voter.connect(governorSigner).governanceWhitelist([boostAddress, usdAddress, poolAddress], {
          gasLimit: 5000000
        });
      } catch (error) {}
      ...
      if (isBoostWhitelisted && isUsdWhitelisted && isPoolWhitelisted) {
        const createGaugeTx = await v2Voter.connect(governorSigner).createGauge(poolAddress, { gasLimit: 5000000 });
    ...
    describe("AMO Role Actions", function () {
      describe("mintAndSellBoost", function () {
        it("Should execute mintAndSellBoost successfully", async function () {
          ...
          let boostAmount = BigInt(Math.sqrt(Number(usdReserve * boostReserve))) - boostReserve;
      ...
      it("Should revert unfarmBuyBurn when called by non-amo", async function () {
        ...
        await expect(v2AMO.connect(user).unfarmBuyBurn(ethers.parseUnits("1000000", 18), 1, 1))
          .to.be.revertedWithCustomError(v2AMO, "AccessControlUnauthorizedAccount")
          .withArgs(user.address, AMO_ROLE);
      });
      ...
      it("should allow WITHDRAWER_ROLE to withdraw ERC20 tokens", async function () {
        ...
        await expect(v2AMO.connect(user).withdrawERC20(usdAddress, ethers.parseUnits("1000", 6), user.address))
          .to.be.revertedWithCustomError(v2AMO, "AccessControlUnauthorizedAccount")
      ...
      it("should execute unfarmBuyBurn succesfully", async function () {
        ...
        await router
          .connect(user)
          .swapExactTokensForTokens(boostToBuy, minUsdReceive, routeSellBoost, user.address, deadline);
      ...
      describe("addLiquidity", function () {
        it("should addLiquidity succesfully ", async function () {
          ...
          try {
            ...
            const tx = await v2AMO.connect(amoBot).addLiquidity(usdAmountToAdd, boostMinAmount, usdMinAmount, {
              gasLimit: 5000000
            });
    ...
    describe("Public AMO Functions", function () {
      describe("mintSellFarm", function () {
        it("should execute public mintSellFarm when price above 1", async function () {
          try {
            ...
            await router
              .connect(user)
              .swapExactTokensForTokens(usdToBuy, minOutput, routeBuyBoost, user.address, deadline);
            ...
            expect(finalPrice).to.be.approximately(ethers.parseUnits("1", 6), ethers.parseUnits("0.01", 6));
          } catch (error) {
            console.log("\nDetailed error information:");
            console.log("Error message:", error.message);
      ...
      describe("should revert when invalid parameters are set", function () {
        for (const i of [1]) {
          it(`param on index ${i}`, async function () {
            ...
            await expect(v2AMO.connect(setter).setParams(...tempParams)).to.be.revertedWithCustomError(
              v2AMO,
              "InvalidRatioValue"
            );
          });
      ...
      describe("get reward", async function () {
        ...
        it("should revert for non-setter", async function () {
          await expect(v2AMO.connect(user).setWhitelistedTokens(tokens, true))
            .to.be.revertedWithCustomError(v2AMO, "AccessControlUnauthorizedAccount")
            .withArgs(user.address, SETTER_ROLE);
        });
        ...
        it("should revert for non-reward_collector", async function () {
          await expect(v2AMO.connect(user).getReward(tokens, true))
            .to.be.revertedWithCustomError(v2AMO, "AccessControlUnauthorizedAccount")
