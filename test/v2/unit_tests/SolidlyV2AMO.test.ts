...
describe("V2AMO", function () {
  ...
  async function setupSolidlyV2Environment() {
    try {
      ...
      const createPairTx = await factory.connect(admin).createPair(boostAddress, usdAddress, true);
      ...
      try {
        await v2Voter.connect(governorSigner).governanceWhitelist(
          [boostAddress, usdAddress, poolAddress],
          { gasLimit: 5000000 }
        );
      } catch (error) {
      }
